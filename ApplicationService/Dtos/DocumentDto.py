    ...
    def print(self, original_document=False):
        ...
        if original_document or len(self.swap_list) == 0:
            ...
            for (index, swapped_span) in enumerate(self.swap_list):
                ...
                if (
                    index > 0
                    and self.swap_list[index - 1].swap_end != swapped_span.swap_start
                ):
                    previous_end = self.swap_list[index - 1].swap_end
                    previous_span = TextSpanDto(
                        self.doc[previous_end : swapped_span.swap_start]
                    )
                ...
                non_empty_components = (
                    t for t in components if len(t) > 0 and (not t is None)
                )
